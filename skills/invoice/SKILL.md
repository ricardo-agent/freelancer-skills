...
name: invoice
description: >
  Generate a clean, client-ready invoice from a plain-English description of the
    work. Use when the user says "make an invoice", "bill this client", "write up
      an invoice for...", or describes hours/rate/expenses that should become an
        invoice. Produces polished HTML (paste straight into an email) plus a matching
          Markdown copy for records. No API keys, no external calls.
          metadata:
            version: 1.0.0
            ---
            ...
            1. A clean **HTML** invoice the user can paste directly into an email (inline
               ...
               - Compute line totals (qty × rate), a subtotal, optional tax (only if the user
                 ...
                 - If the user gives a flat project price with no line items, produce a single
                   ...
                   notes → action items, contract red-flagging, cold outreach, scope-creep replies,
